...
class Pepe extends Agent{
    ...
    playBitCode(board, col) { 
        const new_mask = this.mask | (this.mask + (1n << BigInt(col) * BigInt(board.legnth)));
        const new_position = this.position ^ this.mask;
    ...
    createColumnOrder(cols) {
        let columnOrder = [];
        for (let i = 0; i < cols; i++) {
            columnOrder.push(Math.floor(cols / 2) + (1 - 2 * (i % 2)) * Math.floor((i + 1) / 2));
        }
        return columnOrder;
    }

    heuristic(board, offsets) {
        return 1;
    }

    isFull(board) {
        return this.board.valid_moves(board).length === 0;
    }

    minimax(board, col, depth, offsets, alpha, beta, player, columnOrder) {
        if (depth === 0 || this.isFull(board)) {
            // Generate the score of the current board
            return [this.heuristic(board, offsets), null];
        }
        if (player == 'W') {
            let maxEval = Number.NEGATIVE_INFINITY;
            let bestMove = null;
            for (let col = 0; col < board.length; col++) {
                if (this.board.check(board, columnOrder[col])) {
                    const boardClone = this.board.clone(board);
                    this.board.move(boardClone, columnOrder[col], player);
                    let [evaluation, move] = this.minimax(boardClone, col, depth - 1, offsets, alpha, beta, 'B', columnOrder);
                    if (maxEval < evaluation) {
                        maxEval = evaluation;
                        bestMove = columnOrder[col];
                    }
            ...
            for (let col = 0; col < board.length; col++) {
                if (this.board.check(board, columnOrder[col])) {
                    const boardClone = this.board.clone(board);
                    this.board.move(boardClone, columnOrder[col], player);
                    [this.position, this.mask] = this.playBitCode(board, col);
                    let [evaluation, move] = this.minimax(boardClone, col, depth-1, offsets, alpha, beta, 'W', columnOrder);
    ...
    compute(board, time){
        ...
        let columnOrder = this.createColumnOrder(board.length);
        let [score, index] = this.minimax(board, -1, 3, [], Number.NEGATIVE_INFINITY, Number.POSITIVE_INFINITY, this.color, columnOrder);
        return moves[index]
    }
